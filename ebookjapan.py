...
def scraping(title_list, driver):
    ...
    # サイトURL
    base_url = 'https://ebookjapan.yahoo.co.jp/search/?keyword={}'
    result_list = []
    for title in title_list:
        ...
        # 検索タイトルと一致するものを全て取得
        pre_book_list = []
        books = soup.select('div.contents-search li.item-container__item')[:5]
        for book in books:
            book_title = book.select_one('div.book-item__cover > img').get('alt')
            # 無駄な文字を排除
        ...
        if pre_book_list:
            ...
            hit_title = hit_book.select_one('div.book-item__cover > img').get('alt')
            for pre_book in pre_book_list:
                book_title = pre_book.select_one('div.book-item__cover > img').get('alt')
                if ('分冊版' not in book_title) and ('カラー版' not in book_title) and (hit_title != book_title):
                    hit_book = pre_book
          
            # 完結しているか
            book_condition_tag = hit_book.select_one('div.book-item__caption > p.tagtxt-wrap')
            if book_condition_tag:
                book_condition = book_condition_tag.text
                if '完結' in book_condition:
                    result = '完結'
                
            # 完結していない場合、巻数を確認
            if not result:
                book_title_tag = hit_book.select_one('div.book-item__caption > p.book-item__title')
                book_title = book_title_tag.text.strip()
                k = book_title.rfind(' ')
                if k == -1:
                    k = book_title.rfind('　')
                
                howmany_books = book_title[k+1:-1]
        ...
        input_to_text(title, result, 'a', 'ebookjapan.txt')
...
def main():
    ...
    # データ保存用テキストファイルの更新
    if is_restart == False:
        input_to_text('', '', 'w', 'ebookjapan.txt')
