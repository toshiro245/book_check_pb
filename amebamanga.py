...
def scraping(title_list, driver):
    ...
    # サイトURL
    base_url = 'https://dokusho-ojikan.jp/search/result/1?q={}&sort=monthly_spend&log=true'
    result_list = []
    ...
    for title in title_list:
        ...
        # 検索タイトルと一致するものを全て取得
        pre_book_list = []
        books = soup.select('ul.sc-hxqEdz > li.sc-gfHBtU')[:5]
        for book in books:
            book_title = book.select_one('div.sc-hLGenU div.sc-koaBrS').text.strip()
        ...
        if pre_book_list:
            ...
            hit_title = hit_book.select_one('div.sc-hLGenU div.sc-koaBrS').text
            check_title = hit_title

            for pre_book in pre_book_list:
                book_title = pre_book.select_one('div.sc-hLGenU div.sc-koaBrS').text
                if ('分冊版' not in book_title) and ('カラー版' not in book_title) and (hit_title != book_title):
                    hit_book = pre_book
                    check_title = book_title

            # 完結しているか確認
            howmany_books = hit_book.select_one('div.sc-hLGenU div.sc-cfZFvb > div.sc-lhuRmv').text
            if '完結' in howmany_books:
                result = '完結'
            else:
                if '分冊版' in check_title:
                    result = '分冊版' + howmany_books.replace('既刊', '') + 'まで'
                else:
                    result = howmany_books.replace('既刊', '') + 'まで'
        ...
        input_to_text(title, result, 'a', 'amebamanga.txt')
...
def main():
    ...
    # データ保存用テキストファイルの更新
    if is_restart == False:
        input_to_text('', '', 'w', 'amebamanga.txt')
