...
def scraping(title_list, driver):
    ...
    # サイトURL
    base_url = 'https://www.dmm.co.jp/search/=/searchstr={}/analyze=V1ECC1YHUAE_/limit=30/n1=FgRCTw9VBA4GAV5NWV8I/n2=Aw1fVhQKX1ZRAlhMUlo5Uw4QXF9e/sort=ranking/'
    result_list = []
    over_18_check = False
    ...
    for title in title_list:
        ...
        # 検索タイトルと一致するものを全て取得
        pre_book_list = []
        books = soup.select('#doujin_list > li.productList__item')[:5]
        for book in books:
            book_title = book.select_one('div.tileListImg div.tileListImg__tmb > a > img').get('alt')
            # 無駄な文字を排除
        ...
        if pre_book_list:
            ...
            hit_title = hit_book.select_one('div.tileListImg div.tileListImg__tmb > a > img').get('alt')
            check_title = hit_title

            for pre_book in pre_book_list:
                book_title = pre_book.select_one('div.tileListImg div.tileListImg__tmb > a > img').get('alt')
            ...
            if not result:
                ...
                detail_url = hit_book.select_one('div.tileListImg div.tileListImg__tmb > a').get('href')
                ...
                # 巻数
                howmany_books = 1
                related_works = soup.select('div.productList > div.productList__item')
                for related_work in related_works:
                    related_work_title = related_work.select_one('a > img').get('alt')
                    if check_title in related_work_title:
                        howmany_books += 1

                result = str(howmany_books) + '巻まで'
        ...
        input_to_text(title, result, 'a', 'fanza.txt')
