...
def scraping(title_list, driver):
    ...
    # サイトURL
    base_url = 'https://book.hikaritv.net/book/app/catalog/search/init?searchWord={}'
    result_list = []
    over_18_check = False
    ...
    for title in title_list:
        ...
        # 検索タイトルと一致するものを全て取得
        pre_book_list = []
        books = soup.select('div.listBookArticle ol.bookThum > li')[:5]
        for book in books:
            book_title = book.select_one('span.bookThumData > a > span').text.strip()
        ...
        if pre_book_list:
            ...
            hit_title = hit_book.select_one('span.bookThumData > a > span').text
            check_title = hit_title

            for pre_book in pre_book_list:
                book_title = pre_book.select_one('span.bookThumData > a > span').text
                if ('分冊版' not in book_title) and ('カラー' not in book_title) and (hit_title != book_title):
                    hit_book = pre_book
                    check_title = book_title


            detail_url = 'https://book.hikaritv.net' + hit_book.select_one('span.bookThumData > a').get('href')
            driver.get(detail_url)
            sleep(4)

            if not over_18_check:
                try:
                    driver.find_element(By.CSS_SELECTOR, '#pageNav > ul > li:first-of-type').click()
                    sleep(3)
                    over_18_check = True
                except:
                    pass
            ...
            # 完結済みかどうか
            finish_or_not = detail_soup.select_one('div.sectionReBook > div.status > span.cat-end')
            if finish_or_not:
                result = '完結'

            if not result:
                howmany_books = detail_soup.select_one('div.listNav > div.listNav_inner1 span.js_seriesItemNum').text
        ...
        input_to_text(title, result, 'a', 'hikaritv.txt')
